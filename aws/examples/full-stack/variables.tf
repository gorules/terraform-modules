...
variable "database_auth" {
  description = "Database authentication method: 'secrets' (password) or 'iam' (IAM database auth)"
  type        = string
  default     = "secrets"
}

variable "database_iam_username" {
  description = "Username for IAM database authentication (only used when database_auth='iam')"
  type        = string
...
variable "brms_certificate_arn" {
  description = "ACM certificate ARN for BRMS HTTPS (provide this OR brms_route53_zone_id)"
  type        = string
  default     = null
}

variable "brms_route53_zone_id" {
  description = "Route53 hosted zone ID for automatic certificate creation and DNS (provide this OR brms_certificate_arn)"
  type        = string
  default     = null
}
...
variable "brms_ai_thinking_level" {
  description = "LLM thinking level: high, medium, low"
  type        = string
  default     = "medium"
}
...
variable "agent_domain" {
  description = "Custom domain name for Agent (optional - enables HTTPS when provided with certificate)"
  type        = string
  default     = null
}
