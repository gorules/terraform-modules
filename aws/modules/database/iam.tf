...
data "aws_iam_policy_document" "rds_iam_connect" {
  ...
  statement {
    ...
    resources = ["arn:aws:rds-db:${data.aws_region.current[0].region}:${data.aws_caller_identity.current[0].account_id}:dbuser:${aws_rds_cluster.this.cluster_resource_id}/${var.iam_username}"]
  }
}
